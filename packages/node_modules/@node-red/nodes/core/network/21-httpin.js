...
module.exports = function(RED) {
    ...
    function HTTPIn(n) {
        ...
        if (RED.settings.httpNodeRoot !== false) {
            ...
            //     console.log('push id start');
            //     nodelist.push(n.id);
            //     console.log('push id end');
            //     let string = JSON.stringify(nodelist);
            //     node.context().global.set("monitors00", string);
            //     console.log('tick id set end');

                // node.context().global.set("monitorHttp", [{ "NodeId": n.id }]);
                ...
                if (node.method.match(/^(post|delete|put|options|patch)$/)) {
                    if(n.monitor){
                       console.log("tracing")
                        ...
                        const span = tracer.startSpan("HTTP Request inside");
                     
                      const testdata = req.body
                 console.log(span,"checkspan")
                     span.setAttributes({
                        "http.status_code": res.status,
                        "request.body": JSON.stringify(testdata),
                          });
                          span.end();
                    }
                   
                    // console.log("check0",req.body)
                    node.send({_msgid:msgid,req:req,res:createResponseWrapper(node,res),payload:req.body});
                } else if (node.method == "get") {
                    console.log(node.monitor,"test3")
